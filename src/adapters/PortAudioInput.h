 ...
 * @brief PortAudio implementation of IAudioInput interface
 *
 * This adapter provides real-time audio capture using PortAudio library.
 ...
 * @brief PortAudio-based audio input capture adapter
 *
 * Implements real-time audio capture using PortAudio streams.
 * Audio data is transferred from the audio callback to the read
 * methods via a lock-free SPSC ring buffer.
 *
 * State Management:
 * - Stopped: Initial state, stream not open or closed
 * - Starting: Transition state during Open()
 * - Active: Stream is running and capturing audio
 ...
 * - const methods: RT-safe (AvailableFrames, Read, GetState, IsActive)
 */
...
public:
     ...
     *
     * Starts the PortAudio stream and begins capturing audio.
    ...
    /**
     * @brief Close audio input stream
     *
    ...
    /**
     * @brief Check if stream is actively capturing
     *
     ...
     *
     * Returns the number of frames that can be read without blocking.
     *
    ...
    /**
     * @brief Read audio data from ring buffer
     *
     * Reads up to buffer.frameCount frames from the internal ring buffer.
     * Actual frames read may be less if not enough data available.
     *
     * @param buffer AudioBuffer to fill with audio data
...
private:
     ...
     *
     * @param inputBuffer Input audio data from device
     * @param outputBuffer Not used for input stream
     * @param frameCount Number of frames to process
     * @param timeInfo Timing information
     * @param statusFlags Stream status flags
     * @param userData Pointer to PortAudioInput instance
     ...
     *
     * Processes audio data from PortAudio and writes to ring buffer.
     ...
     * @param statusFlags Stream status flags
     */
    int ProcessAudio(const float* inputBuffer,
                     unsigned long frameCount,
                     PaStreamCallbackFlags statusFlags);
